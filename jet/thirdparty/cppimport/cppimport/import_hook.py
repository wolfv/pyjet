...
def build_plugin(full_module_name, filepath):
    ...
    jet_support_include = os.path.join(dirname, '../../../include/')
    pybind11_include = os.path.join(dirname, '../../pybind11/include/')

    armadillo_install = os.path.join(dirname, '../../armadillo/installed/')
    armadillo_include = os.path.join(armadillo_install, 'include/')
    armadillo_lib = os.path.join(armadillo_install, 'lib/armadillo.so.7')
    ...
    ext = ImportCppExt(
        ...
        extra_compile_args = [
            '-std=c++11', '-w', '-O3', '-ffast-math', '-lblas', '-llapack'
        ]
    )
